...
# import pyromaths
from src.pyromaths.Values import data_dir, configdir
from pyromaths.outils.System import Fiche

# THUMBDIR = os.path.join(data_dir(), "exercices", "img")
data = join(abspath(dirname(__file__)), '../pyromaths/data/')
THUMBDIR = os.path.join(data, "exercices", "img")
...
def main(tempdir):
    ...
    with md5file() as md5sums:
        for i, key in enumerate(sorted(bag.keys())):
            exo = bag[key]
            logging.info("Exercice '%s' (%s/%s).", exo.name(), i, len(bag))
