...
- un exemple simple (avec très peu de cas particuliers) : la recherche d'état stable.
    ...
    - classe :class:`EtatStableSysteme2` du module :download:`matrices.py <../../pyromaths/ex/lycee/matrices.py>`
    - :download:`template de l'énoncé <../../pyromaths/data/exercices/templates/EtatStableSysteme2-statement.tex>`
    - :download:`template de la solution <../../pyromaths/data/exercices/templates/EtatStableSysteme2-answer.tex>`

- un exemple plus complexe (avec cas particuliers) : bilan sur les polynômes du second degré en seconde.

    - classe :class:`BilanTrinomeSansDiscriminant` du module :download:`SecondDegre.py <../../pyromaths/ex/lycee/SecondDegre.py>` ;
    - :download:`template de l'énoncé <../../pyromaths/data/exercices/templates/BilanTrinomeSansDiscriminant-statement.tex>` ;
    - :download:`template de la solution <../../pyromaths/data/exercices/templates/BilanTrinomeSansDiscriminant-answer.tex>`.
...
À part `git <http://git-scm.com>`__, tous les outils nécessaires pour créer un exercice sont des dépendances de Pyromaths. Installez `la version de développement <https://framagit.org/pyromaths/pyromaths/#d%C3%A9velopper-pyromaths>`__, et faites fonctionner Pyromath : les outils nécessaires pour ce tutoriel seront alors disponibles.

Environnement de travail
========================

Commençons par télécharger les sources de Pyromaths, en utilisant le logiciel `git`. Si vous avez un compte `Framagit <http://framagit.com>`__, utilisez :
...
Ce fichier doit être compilé avec ``lualatex``. À la fin de cette étape, nous obtenons l'énoncé suivant (:download:`tex <ecrire/1/exercices.tex>`, :download:`pdf <ecrire/1/exercices.pdf>`).
...
Le code Python de l'exercice doit être placé dans un des sous-dossiers de ``pyromaths/ex/``. Dans notre cas, ce sera ``pyromaths/ex/troisiemes``. Ensuite, modifiez un des fichiers `.py` déjà existant, ou créez-en un nouveau. Gardez une certaine logique : un exercice sur Pythagore a sa place dans le même fichier qu'un autre exercice sur Pythagore ; un exercice de trigonométrie n'a pas sa place dans un fichier ``matrices.py``. Dans notre cas, nous créons un nouveau fichier contenant le code suivant.
...
- ligne 30 : les tags (étiquettes) de l'exercice (ceux-ci doivent contenir le niveau, plus d'autres à votre guise ; pour connaître la liste des tags déjà utilisés, utilisez ``pyromaths tags``). Vous pouvez laisser la liste vide pour le moment : nous y reviondrons :ref:`plus tard <ecrire-tags>`.

Code :math:`LaTeX`
------------------

Le code :math:`LaTeX`, quant à lui, doit être placé dans le répertoire ``pyromaths/data/exercices/templates``, dans deux fichiers au nom de votre exercices. Reprenez votre fichier :download:`exercices.tex <ecrire/1/exercices.tex>`, et extrayez les lignes correspondant à l'énoncé, que vous écrivez dans le fichier :download:`EquationPremierDegre2-statement.tex <ecrire/2/EquationPremierDegre2-statement.tex>`, et celles correspondant au corrigé dans le fichier :download:`EquationPremierDegre2-answer.tex <ecrire/2/EquationPremierDegre2-answer.tex>`.
...
.. literalinclude::  ecrire/3/equation3.py
   :linenos:
   :pyobject: EquationPremierDegre3.__init__
...
.. note::
   ...
   Notons que les chaînes définissant ces blocs ont été modifiées par rapport aux chaînes initiales, car trop proches de la syntaxe :math:`LaTeX`. Ceci est documenté sur `le site officiel <http://jinja.pocoo.org/docs/2.10/api/#jinja2.Environment>`__, et mis en œuvre dans la classe :class:`pyromaths.outils.jinja2utils.LatexEnvironment`.

   .. literalinclude:: ../../pyromaths/outils/jinja2utils.py
      :linenos:
      :pyobject: LatexEnvironment
...
Il suffit d'ajouter cette fonction à l'attribut :attr:`~pyromaths.ex.troisiemes.equation.EquationPremierDegre4.filters` de la classe :class:`~pyromaths.ex.troisiemes.equation.EquationPremierDegre4`, qui est un dictionnaire de tous les filtres utilisés par notre *template* :

.. literalinclude::  ecrire/5/equation5.py
   :linenos:
   :lines: 1-5
   :emphasize-lines: 5
   :pyobject: EquationPremierDegre5
...
Remarquons enfin que si d'autres ajouts doivent être faits à :class:`l'environnement <jinja2.Environment>`, il est possible de surcharger la méthode :meth:`pyromaths.ex.Jinja2Exercise.environment` :
...
Pour éviter ce genre d'arrondi, il est possible d'utiliser le type :class:`decimal.Decimal` pour les nombres à virgule. Cette classe est conçue comme étant une représentation « pour les humains » d'un nombre à virgule (contrairement au type :class:`float`, qui est une représentation « pour les ordinateurs »), et arrondi correctement les calculs. Nous n'en avons *a priori* pas besoin ici.
...
D'autres outils ont été développés pour faciliter l'écriture du LaTeX.

*Filter* :func:`~pyromaths.outils.jinja2utils.tex`
--------------------------------------------------

Le *filter* :func:`~pyromaths.outils.jinja2utils.tex` est intégré par défaut à l'invironnement. Il permet d'écrire *convenablement* certains objets : les :class:`nombres <numbers.Real>`, et d'autres objets qui implémentent la méthode :meth:`tex` (comme :class:`~pyromaths.classes.ensembles.Intervalle` par exemple).

Par exemple, pour afficher une :class:`~fractions.Fraction`, le code ``(( Fraction(1, 3)|tex ))`` produira ``\frac{\numprint{1}}{\numprint{3}}``.

Et bien d'autres
----------------

Des classes et fonctions ont été développées pour manipuler, détailler les calculs, afficher diverses objets mathématiques :
:mod:`vecteurs <pyromaths.classes.Vecteurs>`,
:mod:`intervalles (et unions d'intervalles) <pyromaths.classes.ensembles>`,
:mod:`équations et inéquations du premier degré <pyromaths.classes.premierdegre>`,
etc.

Allezvoir :ref:`la liste complète <api>`.


Gestion des cas particuliers
============================

Revenons à notre exercice : deux problèmes subsistent.
...
.. literalinclude::  ecrire/6.1/equation61.py
   :linenos:
   :pyobject: EquationPremierDegre61
...
.. literalinclude::  ecrire/6.2/equation62.py
   :linenos:
   :pyobject: EquationPremierDegre62
...
.. literalinclude::  ecrire/6.3/equation63.py
   :linenos:
   :pyobject: EquationPremierDegre63.__init__
...
Pour ajouter des tags à votre exercice (pour faciliter son classement et sa recherche), vous pouvez modifier l'attribut ``tags``. Il est plus simple d'utiliser l'outil :ref:`bidouilles <bidouilles>` :

.. code-block:: shell

   ./utils/bidouilles tag edit EquationPremierDegre

Cet outil va ouvrir un éditeur de texte, pour :

- choisir parmi les tags déjà existants lesquels ajouter à votre exercice ;
- créer de nouveaux tags.

Le fichier Python de votre exercice est alors automatiquement modifié avec les tags choisis.

Ajouter des tests
-----------------

Dans quelques mois ou années, quelqu'un (peut-être vous) voudrait modifier quelque chose de Pyromaths, en se demandant si cela va « casser » un exercice. Pour être sûr que votre exercice soit préservé, il serait sage de le tester. L'ajout d'un test se fait avec la commande suivante (:ref:`plus d'info sur l'outil <bidouilles>`).

.. code-block:: shell

   $ utils/bidouilles test create EquationPremierDegre
...
.. code-block:: shell
   ...
   $ utils/bidouilles test create EquationPremierDegre:1729

Plus tard, pour vérifier que votre exercice n'a pas été modifier, vérifiez les tests en utilisant ce même programme.

.. code-block:: shell

   $ utils/bidouilles test check

Annoncer le nouvel exercice
---------------------------

Vous devez aussi modifier le fichier `NEWS <https://framagit.org/pyromaths/pyromaths/-/blob/develop/NEWS>`__ (à la racine du projet), pour que ce nouvel exercice apparaisse dans le journal des modifications.
...
- deux fichiers de template :math:`LaTeX` (dans le dossier ``pyromaths/data/exercices/templates``) ;
...
Il ne reste qu'à nous proposer votre exercice, de préférence en utilisant une *pull request* sur Gitlab, à défaut en prenant contact avec nous sur `le forum <http://forum.pyromaths.org>`__.
