...
def argument_parser():
    ...
    generate_parser.add_argument(
        ...
        help='Name of the exercises to generate. The special value "ALL" means: "generate all known exercises (this may be used for testing purposes).'
        )
    generate_parser.add_argument(
        '-p', '--pipe',
        nargs=1,
...
def do_generate(options):
    ...
    for exercise, seeds in options.exercise:
        ...
        if exercise == "ALL":
            # Special case: add ALL exercises
            for name in sorted(bag, key=str.lower):
                for seed in seeds:
                    exercise_list.append(bag[name](seed))
        else:
            for seed in seeds:
    ...
    with Fiche(parametres, verbose=options.verbose, dirty=options.dirty) as fiche:
        # LatexmkRC
