...
def _cercle_trigonometrique():
    ...
    while len(mes_deg)<5:
        ...
        if tmp not in mes_deg:
            mes_deg.append(tmp)
    mes_deg_tex = [_(u"$\\ang{%s}$") %(d) for d in mes_deg]
    ...
    cor.append(_(u"Après simplification, voici les résultats : \\ang{%s}, \\ang{%s}, \\ang{%s}, \\ang{%s} et \\ang{%s}.") %(tuple([rad2deg(r) for r in radq2])))
    ...
    cor.append(_(u"Une mesure d'angle en radians est définie modulo $2\\pi$, c'est-à-dire que l'ajout ou la suppression d'un tour ( qui vaut $2\\pi$ ou \\ang{360} ) ne change pas un angle.\\par"))
