...
from pyromaths.outils.tikz import axis, interpolation
#===============================================================================
...
def remplissage_tableau(lX, lY, variations):
    ...
    lAbsc = [v[1] for v in variations] + [variations[-1][2]]
    lOrdo = []
...
class Vf1SensEtTableau(ex.TexExercise):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lX = lX
        self.lY = [None, None]
        self.lY[0], self.variations1 = PointsCourbes(sens_var=(-1, 1))
        self.lY[1], self.variations2 = PointsCourbes()


    def tex_statement(self):
        axes = axis(x=(-5, 5), y=(-4, 4))
        exo = [r'\exercice']
        ...
        for figure in range(2):
            exo.append(axes.begin())
            exo.append(fr"""\draw (axis cs:{self.lX[0]}, {self.lY[figure][0]}) node[left]{{$\mathcal{{C}}{"fg"[figure]}$}};""")
            exo.append(r"""\draw[ultra thick, Maroon] {};""".format(interpolation(zip(self.lX, self.lY[figure]), systeme="axis cs:", tension=.2)))
            exo.append(axes.end())
            if figure == 0:
...
class Vf2ExtremaGraphiques(ex.TexExercise):
    ...
    def __init__(self, *args, **kwargs):
        ...
        while not fin:  # Évite les boucles infinies
            ...
            for dummy in range(20):
                ...
                [gmin, gmax] = extrema(self.lX, self.lY[0], [-5, 5])
                [lmin, lmax] = extrema(self.lX, self.lY[0], intervalle1)
            ...
            if fin:
                ...
                for dummy in range(20):
                    ...
                    [gmin, gmax] = extrema(self.lX, self.lY[0], [-5, 5])
                    [lmin, lmax] = extrema(self.lX, self.lY[0], intervalle2)
                    if abs(lmin[0] - lmax[0]) > 1 and lmin[1] != gmin[1] and lmax[1] != gmax[1]:
                        fin = True
                        break
    ...
    def tex_answer(self):
        exo = [r'\exercice*']
        exo.append(r'\begin{enumerate}')
        extr = extrema(self.lX, self.lY[0], [self.lX[0], self.lX[-1]])
        exo.append(u'\\item ')
        ...
        extr = extrema(self.lX, self.lY[0], self.intervalle1)
        ...
        extr = extrema(self.lX, self.lY[1], [self.lX[0], self.lX[-1]])
        exo.append(u'\\item ')
        exo.append(r'\begin{itemize}[leftmargin=*]')
        ...
        exo.append(r'\end{itemize}')
        extr = extrema(self.lX, self.lY[1], self.intervalle2)
        exo.append(u'\\item ')
        exo.append(r'\begin{itemize}[leftmargin=*]')
...
class Vf3VariationVersCourbe(ex.TexExercise):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lX = list(lX)
        self.lY = [None, None]
        self.variations = [None, None]
        self.lY[0], self.variations[0] = PointsCourbes(sens_var=(-1, 1))
        self.lY[1], self.variations[1] = PointsCourbes()
    ...
    def tex_statement(self):
        ...
        exo.append(r'\begin{tabular}[t]{ll}')
        for j in range(2):
            variations = [self.variations[0], self.variations[1]][j]
            lY = [self.lY[0], self.lY[1]][j]
            exo.append(r'\begin{tikzpicture}[scale=1]')
            exo.append(r'\tkzTabInit[lgt=1.2,espcl=1.2]{$x$/1,$%s\,(x)$/3}{$%s$}' % (['f', 'g'][j], '$,$'.join([str(v[1]) for v in variations] + [str(variations[-1][2])])))
            ans = r'\tkzTabVar{'  # %-/$-2$,+/$5$,-/$-3$,+/$-1$,-/$-4$}'
            trois_niv = False
    ...
    def tex_answer(self):
        ...
        exo.append(r'\begin{center}')
        for figure in range(2):
            lX, lY = integration_zeros(self.lX, self.lY[figure], self.variations[figure])
            exo.append(axes.begin())
            exo.append(fr"""\draw (axis cs:{lX[0]}, {lY[0]}) node[left]{{$\mathcal{{C}}{"fg"[figure]}$}};""")
            exo.append(r"""\draw[ultra thick, Maroon] {};""".format(interpolation(zip(lX, lY), systeme="axis cs:", tension=.4)))
            exo.append(axes.end())
            if figure == 0:
                exo.append(r'\kern1cm')
        exo.append(r'\end{center}')
