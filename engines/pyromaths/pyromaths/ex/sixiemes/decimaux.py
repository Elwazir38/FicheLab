...
TIKZSTYLE = '\\tikzstyle{fleche}=[overlay, line width=0.6pt, dotted, -Stealth, Maroon]'
...
def tex_tableau(cor, div0, div1, u, nblist, chf_unite):
    ...
    nblist[div0] = "%s\\tikzmarknode{virg0}{\\ }" % nblist[div0]
    nblist[div1] = "{%s\\tikzmarknode{virg1}{\\textcolor{red}{ \\LARGE ,}}}" % nblist[div1]

    # ajoute au tabular la ligne avec 6 ou 7 colonnes
    cor.append(r"\stepcounter{tikzmark}")
    cor.append(("%s " + ("& %s"*(6 - delta))) % tuple(nblist))
    cor.append(f"\\tikz[remember picture] \\draw[fleche] ($(pic cs:virg0)-(0, 1mm)$) -- ($(pic cs:virg1)-(0, 1mm)$); \\\\")
...
def tex_conversion(exo, cor, exposant, u):
    ...
    exo.append("\\item $\\SI{%s}{%s}=\\dotfill\\si{%s}$" % 
            (Affichage.decimaux(nb0), u[div0], u[div1]))
    cor.append("\\item $\\SI{%s}{%s}=\\SI{%s}{%s}$\\vspace{1ex}\\par" % 
...
def tex_tableau_conversion(div0, div1, nb0, u, exposant):
    ...
    nblist[exposant * (div0 + 1) - 1] = "%s\\tikzmarknode{virg0}{\\ }" % nb_dict.get(exposant * (div0 + 1) - 1, "0")
    nblist[exposant * (div1 + 1) - 1] = "{%s\\tikzmarknode{virg1}{\\virgule}}" % nb_dict.get(exposant * (div1 + 1) - 1, "0")
    return [
            r"\stepcounter{tikzmark}",
            ("%s " + "& %s"*(7 * exposant - 1)) % tuple(nblist),
            f"\\tikz[remember picture] \\draw[fleche] ($(pic cs:virg0)-(0, 1mm)$) -- ($(pic cs:virg1)-(0, 1mm)$); \\\\",
            ]
