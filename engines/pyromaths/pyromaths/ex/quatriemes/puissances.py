...
def _exo_puissances():
    ...
    exo.append("\\[ \\thenocalcul = %s \\]" % tex_puissances_0(valeurs[1 - i]).translate(sd))
    cor.append("\\[ \\thenocalcul = %s \\]" % tex_puissances_0(valeurs[1 - i]).translate(sd))
    cor.append("\\[ \\thenocalcul = %s \\]" % tex_puissances_1(valeurs[1 - i]).translate(sd))
    cor.append("\\[ \\thenocalcul = %s \\]" % tex_puissances_2(valeurs[1 - i]).translate(sd))
    if int(floor(log10(valeurs[1 - i][0] * valeurs[1 - i][1] / valeurs[1 - i][2]))) != 0:
        cor.append("\\[ \\thenocalcul = %s \\]" % tex_puissances_3(valeurs[1 - i]).translate(sd))
        cor.append("\\[ \\boxed{\\thenocalcul = %s} \\]" % tex_puissances_4(valeurs[1 - i]).translate(sd))
    exo.append("\\end{multicols}\n")
    cor.append("\\end{multicols}\n")
    return (exo, cor)
...
def tex_puissances_0(a):
    if isinstance(a, tuple):
        return '\\cfrac{\\nombre{%s} \\times 10^{%s} \\times \\nombre{%s} \\times 10^{%s}}{\\nombre{%s} \\times \\big( 10^{%s} \\big) ^%s}' % \
            (decimaux(a[0]), decimaux(a[3]), decimaux(a[1]), decimaux(a[4]), decimaux(a[2]), decimaux(a[5]), decimaux(a[6]))
    ...
    if isinstance(a, tuple):
        if a[4] < 0:
            return '\\cfrac{\\nombre{%s} \\times \\nombre{%s}}{\\nombre{%s}} \\times \\cfrac{10^{%s+(%s)}}{10^{%s \\times %s}}' % \
                (decimaux(a[0]), decimaux(a[1]), decimaux(a[2]), decimaux(a[3]), decimaux(a[4]), decimaux(a[5]), decimaux(a[6]))
        else:
            return '\\cfrac{\\nombre{%s} \\times \\nombre{%s}}{\\nombre{%s}} \\times \\cfrac{10^{%s+%s}}{10^{%s \\times %s}}' % \
                (decimaux(a[0]), decimaux(a[1]), decimaux(a[2]), decimaux(a[3]), decimaux(a[4]), decimaux(a[5]), decimaux(a[6]))
...
def tex_puissances_2(a):
    if isinstance(a, tuple):
        if a[0] * a[1] / a[2] == a[0] * a[1] // a[2]:
            if a[5] * a[6] < 0:
                return '\\nombre{%s} \\times 10^{%s-(%s)}' % \
                       (decimaux((a[0] * a[1]) // a[2]), decimaux(a[3] + a[4]), decimaux(a[5] * a[6]))
            else:
                return '\\nombre{%s} \\times 10^{%s-%s}' % (decimaux(a[0] * a[1] / a[2]), decimaux(a[3] + a[4]), decimaux(a[5] * a[6]))
        else:
            if a[5] * a[6] < 0:
                return '\\nombre{%s} \\times 10^{%s-(%s)}' % \
                       (decimaux(a[0] * a[1] / a[2]), decimaux(a[3] + a[4]), decimaux(a[5] * a[6]))
...
def tex_puissances_3(a):
    ...
    if isinstance(a, tuple) and b != 0:
        return '\\nombre{%s}  \\times 10^{%s} \\times 10^{%s}' % \
               (decimaux(a[0] * a[1] / a[2] / 10 ** b), decimaux(b), decimaux(a[3] + a[4] - a[5] * a[6]))
...
def tex_puissances_4(a):
    ...
    b = int(floor(log10(a[0] * a[1] / a[2])))
    if isinstance(a, tuple):
        return '\\nombre{%s}  \\times 10^{%s}' % (decimaux(a[0] * a[1] / a[2] / 10 ** b), decimaux(b + a[3] + a[4] - a[5] * a[6]))
