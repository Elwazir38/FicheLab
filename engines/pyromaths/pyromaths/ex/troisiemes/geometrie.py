...
def valeurs_thales(valeurmax, typeexo):
    ...
    for i in range(3):
        if liste[i]:
            valeurs[i] = random.randrange(15, valeurmax) / 10
        if liste[i + 3] and liste[i]:
            valeurs[i + 3] = random.randrange(5, valeurs[i] * 10 - 9) / 10
        elif liste[i + 3]:
            valeurs[i + 3] = random.randrange(5, valeurmax) / 10
    if liste[6]:
        valeurs[6] = random.randrange(5, valeurmax) / 10
    if liste[7]:
        valeurs[7] = random.randrange(5, valeurmax) / 10
...
def test_valeurs_thales(valeurs, rapport, type_thales):
    ...
    if v[rapport[0] % 3]:  # rapport est AE/AB
        rapp = v[rapport[0] % 3 + 3] / v[rapport[0] % 3]
    else:
        rapp = 0
    for i in range(3):
        if not v[i] and rapp:
...
def tex_enonce_thales(noms, valeurs, text_arrondi):
    ...
    texte += \
        'On donne $%s=\\SI{%s}{cm}$ \\quad $%s=\\SI{%s}{cm}$ \\quad $%s=\\SI{%s}{cm}$ \\quad $%s~=~\\SI{%s}{cm}$.\\par\n' % \
        tuple(liste[0:8])
...
def valeur_exacte(a, approx=3, unit=1):
    ...
    if unit:
        if nb.count(',') and (len(nb) - nb.find(',')) - 1 > approx:
            return '\\simeq\\SI{' + decimaux(round(a, approx)) + '}{cm}'
        else:
            return '=\\SI{' + decimaux(a) + '}{cm}'
    else:
        if nb.count(',') and (len(nb) - nb.find(',')) - 1 > approx:
...
def fig_thales(noms, valeurs):
    ...
    maxi_y = int((100.0 * v[1]) * sin(float(angle) * pi / 180)) / 100 + .5
...
def tex_fig_thales(noms, valeurs):
    ...
    return textwrap.dedent(r"""
        \begin{{tikzpicture}}[scale={echelle}, very thick]
            ...
            \draw[Maroon] (C) -- (B);
            \path
                ($(A)+({angleA}:2em)$) node {{ {nomA} }}
                ($(B)+({angleB}:2em)$) node {{ {nomB} }}
                ($(C)+({angleC}:2em)$) node {{ {nomC} }}
                ($(b)+({angleb}:2em)$) node {{ {nomb} }}
                ($(c)+({anglec}:2em)$) node {{ {nomc} }}
                ;
        \end{{tikzpicture}}
...
def valeurs_reciproque_thales():
    while True:
        ...
        if c < d:
            (c, d) = (d, c)
        if a != b and int(c / d) != c / d and 10 < a * c < 200 and 10 < a * d < 200 and 10 < b * c < 200 and \
                10 < b * d < 200 and .3 < d / c < .7:
            break
...
def fig_rec_thales(noms, v):
    type_thales = v[9]
    angle = v[8]
    mini_x = int(100 * min(0, v[1] * cos(angle * pi / 180), v[3] * type_thales,
                           (v[4] * cos(angle * pi / 180)) * type_thales)) / 100 - 1.5
    mini_y = int(100 * min(0, (v[4] * sin(angle * pi / 180)) * type_thales)) / 100 - 1.5
    maxi_x = int(100 * max(v[0], v[1] * cos(angle * pi / 180))) / 100 + 1.5
    maxi_y = int(100 * v[1] * sin(angle * pi / 180)) / 100 + .5
    echelle = int(400 / max(abs(mini_x) + maxi_x, abs(mini_y) + maxi_y)) / 100
    if type_thales == 1:
        return {
                "echelle": echelle,
...
def tex_fig_rec_thales(noms, valeurs):
    ...
    return textwrap.dedent(r"""
        ...
        \begin{{tikzpicture}}[scale={echelle}, very thick]
            ...
            \draw[Maroon] (C) -- (B);
            \path
                ($(A)+({angleA}:{distancelegende})$) node {{ {nomA} }}
                ($(B)+({angleB}:{distancelegende})$) node {{ {nomB} }}
                ($(C)+({angleC}:{distancelegende})$) node {{ {nomC} }}
                ($(b)+({angleb}:{distancelegende})$) node {{ {nomb} }}
                ($(c)+({anglec}:{distancelegende})$) node {{ {nomc} }}
                ;
        \end{{tikzpicture}}
        \end{{minipage}}""").format(**donnees)
...
def tex_enonce_rec_thales(n, v):
    ...
    texte = \
        r"""\begin{minipage}{.7\linewidth}
        Sur la figure ci-contre, on donne $%s=\SI{%s}{cm}$, $%s=\SI{%s}{cm}$, $%s=\SI{%s}{cm}$ et $%s=\SI{%s}{cm}$.\par
...
def valeurs_trigo():
    l = [random.randrange(10, 121) / 10 for i in range(3)]
    l.sort()
