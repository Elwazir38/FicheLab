...
from pyromaths.outils import Priorites3, tikz
...
def doublefleche(A, B):
    ...
    res1 = "\\draw[dashed, -Stealth, Maroon, line width=1.1pt] {} -> {};\n".format(A, mid)
    res2 = "\\draw[dashed, -Stealth, Maroon, line width=1.1pt] {} -> {};\n".format(mid, B)
    res = res1 + res2
    if A == B:
        res = ""
...
def tracedroite(A, B, xmin, xmax, ymin, ymax):
    ...
    return "\\draw[Maroon] {} -- {};".format(l[0], l[1])
...
def tracefonc(f, i, A, B, xmin, xmax, ymin, ymax):
    ...
    l = [u'Tracer la droite représentative ($d_' + str(i) + '$) de la fonction $' + f + ':x\\longmapsto ' + str(
        Polynome([[u, 1], [A[1], 0]], "x")) + '$.',
         ...
         rf'\draw {A} node[Maroon]{{\times}};',
         rf'\draw {B} node[Maroon]{{\times}};',
         ]
    return l
...
def exprfonc(f, i, A, B):
    ...
    l = [u'Déterminer l\'expression de la fonction $' + f + u'$ représentée ci-contre par la droite ($d_' + str(
        i) + '$).',
         ...
         rf'\draw {mid1} node {{( {deltay} )}};',
         rf'\draw {mid2} node {{( {deltax} )}};']
...
def _affine():
    ...
    exo = ["\\exercice", "\\begin{minipage}{0.5\\linewidth}",
    ...
    cor = ["\\exercice*", "\\setlength{\\columnsep}{2mm}",
           ...
           "\\end{multicols}",
           "\\vspace{0.45cm}",
           "\\begin{tikzpicture}[scale=.8, baseline=(O.base)]",
           tikz.grid(xmin, ymin, xmax, ymax),
           tracedroite(A, B, xmin, xmax, ymin, ymax),
           noms[0],
