...
def tex_puissances_3(a):
    ...
    b = Decimal(int(floor(log10(a[0] * a[1] / a[2]))))
...
def verifie_type(
    ...
    for i in range(len(a)):
        if str(a[i]).endswith('.0'):
            list.append(round(a[i] + Decimal(".1")))
        else:
            list.append(a[i])
    return tuple(list)
...
def valeurs_puissances():  # renvoie un tuple contenant les valeurs pour les deux exercices sur les puissances
    ...
    while True:
        ...
        if b1 != 1 and b2 != 1:
            break
    while True:
        (n1, n2) = ((b1 * valeur_alea(2, maxi)) * Decimal(10) ** randrange(-emax, emax),
                    (b2 * valeur_alea(2, maxi)) * Decimal(10) ** randrange(-emax, emax))
        n3 = ((b1 * b2) * choice((2, 4, 5, 8))) * Decimal(10) ** randrange(-emax, emax)
    ...
    while True:
        (b1, b2) = (Decimal(valeur_alea(2, maxi)), Decimal(valeur_alea(2, maxi)))
        (b1, b2) = (b1 // pgcd(b1, b2), b2 // pgcd(b1, b2))
        if b1 != 1 and b2 != 1:
            break
    (n1, n2) = ((b1 * valeur_alea(2, maxi)) * Decimal(10) ** randrange(-emax, emax + 1),
                (b2 * valeur_alea(2, maxi)) * Decimal(10) ** randrange(-emax, emax + 1))
    n3 = ((b1 * b2) * choice((1, 2, 4, 5, 8))) * Decimal(10) ** randrange(-emax, emax + 1)
