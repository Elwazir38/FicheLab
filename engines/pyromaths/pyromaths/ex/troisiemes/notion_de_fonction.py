...
from pyromaths.outils.tikz import axis
...
def _notion_fonction():
    ...
    exo.append(u"\\item Le graphique ci-dessous représente une fonction $k$ : \\par")
    # Graphique
    axes = axis((-4, 4), (-4, 4))
    graphique_debut = (
            r"\begin{center}"
            + axes.begin()
            + fr"\addplot[line width=1.5pt, smooth, color=DarkRed, domain=-4.2:4.2]{{{p}}};"
            )
    for x, y in val_exo3:
        graphique_debut += fr"""\draw (axis cs:{x}, {y}) node[Maroon]{{$\times$}};""" + "\n"
    graphique_fin = (
            axes.end()
            + r"\end{center}"
            )

    exo.append(graphique_debut)
    exo.append(graphique_fin)
    cor.append(graphique_debut)
    lpos31 = [i for i in range(5)]
    for dummy in range(1):
        del lpos31[randrange(len(lpos31))]
    for i in range(4):
        x, y = val_exo3[lpos31[i]]
        cor.append(fr"""\draw[dashed, DarkRed, thick] (axis cs:{x}, 0) -- (axis cs:{x}, {y}) -- (axis cs:0, {y});""")
    cor.append(graphique_fin)
    ## Fin du graphique
