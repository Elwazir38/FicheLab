...
def diagramme_tex(typed=2, val=[[], []], aide=0):
    ...
    if typed == 1:  # Diagramme en bâtons
        ...
        grad = len(val[1]) + 1
        legendey = _("Fréquences (\%)") # Obligé de la définir avant car les \ ne sont pas autorisés dans les f-strings
        diag = fr"""\begin{{center}}\begin{{tikzpicture}}
        \begin{{axis}}[
            axis lines=center,
            enlargelimits=upper,
            axis line style={{Maroon,-Stealth}},
            ymin=0, ystep=10,
            %height=10cm,
            width=10cm,
            y=1mm,
            grid=both,
            xmin=0, xmax={len(val[1])-1}, xtick={{1, 2, ..., {len(val[1])-1} }},
            xlabel={{ {_("Valeurs")} }},
            x label style={{at={{(axis description cs:1,0)}},anchor=west}},
            ylabel={{ {legendey} }},
            y label style={{at={{(axis description cs:0,1)}},anchor=south}},
            after end axis/.code={{\path (axis direction cs:0,0) node [below left] {{0}}; }}
            ]
        \addplot+[green, fill=green, mark=none, ybar, bar width=3pt] coordinates {{"""
    ...
    elif typed in (2, 3):  # Diagramme circulaire ou semi-circulaire
        if typed == 2:
            angletotal = 360
        else:
            angletotal = 180
        ...
        angle_cumulé = 0
        angles = set()
        for i, pourcent in enumerate(val[1]):
            angles.add(angle_cumulé)
            angle = round(angletotal * pourcent / 100)
            diag += fr"""
            \draw[Maroon, fill={couleur[i%12]}, smooth cycle] (0, 0) -- ({angle_cumulé}:1) arc ({angle_cumulé}:{angle_cumulé+angle}:1);
            """
            angle_cumulé += angle

        if aide:
            pas = angletotal // pgcd(*(round(angletotal * pourcent / 100) for pourcent in val[1]))
            for i in range(0, angletotal, angletotal//pas):
                if i not in angles:
                    diag += fr"""
                    \draw[dashed, gray] (0, 0) -- ({i}:1);
                    """

        angle_cumulé = 0
        for i, item in enumerate(zip(val[0][1:], val[1])):
            légende, pourcent = item
            angle = round(angletotal * pourcent / 100)
            if angle:
                diag += fr"""
                \draw ({angle_cumulé + angle/2}:1) node[anchor=east, rotate={angle_cumulé + angle/2}]{{ {légende} }};
                """
            angle_cumulé += angle
...
def exo_ages():
    ...
    diagramme = diagramme_tex(choix_diagramme + 2, [[_(u"Ages"), _(u"<20 ans"), _("20 - 40 ans"), _("40 - 60 ans"), _("60 - 80 ans"), _(u">80 ans")], frequences], 1)
    ...
    cor.append(_(u"Sachant que la classe des moins de vingt ans est composée de ") + str(frequences[0]) + u" \\% de " + str(population) + _(u" personnes, on peut calculer l'effectif concerné :\\par"))
...
def exo_vote():
    ...
    for f in range(4):
        if f == idx:
            ...
            frequences.append(-1)
    prenoms = [_(u"Vincent"), _(u"Sophia"), _(u"Karim"), _(u"Denise"), _(u"Aline"), _(u"Jonathan"), _(u"Isabelle"), _(u"Thomas"), _(u"Léna"), _(u"Matteo"), _(u"Céline"), _(u"Antoine"), _(u"Julie"), _(u"Rémy"), _(u"Caroline"), _(u"Yann"), _(u"Muriel"), _(u"Patrick"), _(u"Mélanie")]
    random.shuffle(prenoms)
    titres = [_(u"Elève")] + prenoms[:4]
...
def exo_sport():
    ...
    cor.append(fr"""\begin{{minipage}}{{10cm}}
    \begin{{tikzpicture}}
    \begin{{axis}}[
        axis lines=center,
        enlargelimits=upper,
    ...
    \addplot+[Maroon, fill=Peru, ybar interval, mark=no] coordinates {{ (0, {h1}) (1, {h2}) (2, {h3}) (3, {h4}) (4, {h5}) (5, {h6}) (6, {h7}) (7, 0) }};
    \end{{axis}}
    \end{{tikzpicture}}
    \end{{minipage}}
    \begin{{minipage}}{{6cm}}
    {_("Sur l'axe horizontal, on représente les durées en heures et, sur l'axe vertical, on représente les effectifs.")}
    \end{{minipage}}""")
    cor.append(question2)
    cor.append(_(u"L'effectif total est égal à $ %s + %s + %s + %s = 30$. La mesure d'angle d'un secteur circulaire est proportionnelle à l'effectif du sport qu'il représente. Le coefficient de proportionnalité est égal au quotient de l'effectif total par \\ang{360} c'est à dire $360 \\div 30=12$.\\par") % (basket, tennis, football, judo))
    cor.append(_(u"""\\renewcommand\\tabcolsep{10pt}
    \\begin{tabular}{|l|c|c|c|c|c|c}
    \\cline{1-6}
    Sport favori  & Basket-ball & Tennis & Football & Judo & Total &\\\\
    \\cline{1-6}
    Effectif & %s & %s & %s & %s & 30 &\\stepcounter{tikzmark}\\tikzmarknode{plan1}{\strut}\\\\
    \\cline{1-6}
    Mesure (en degré)  & \\bf%s & \\bf%s & \\bf%s & \\bf%s & 360 &\\tikzmarknode{plan2}{\strut}%%
            \\tikz[overlay, Maroon, remember picture, line width=1pt, -Stealth, rounded corners=.5em] \\draw (plan1.east) -- ++(.5, 0) |- node[black,pos=.25, right]{$\\times12$} (plan2.east) ; \\\\
    \\cline{1-6}
    \\end{tabular}
    ...
    cor.append(fr"""\begin{{minipage}}{{13cm}}
    \begin{{center}}
    \begin{{tikzpicture}}[thick, scale=3]
        \draw[Maroon, fill=Bisque] (0, 0) -- (0:1) arc (0:{basket*12}:1) -- cycle;
        \draw[Maroon, fill=LightSalmon] (0, 0) -- ({basket*12}:1) arc ({basket*12}:{(basket+tennis)*12}:1) -- cycle;
        \draw[Maroon, fill=Chocolate] (0, 0) -- ({(basket+tennis)*12}:1) arc ({(basket+tennis)*12}:{(basket+tennis+football)*12}:1) -- cycle;
        \draw[Maroon, fill=Peru] (0, 0) -- ({(basket+tennis+football)*12}:1) arc ({(basket+tennis+football)*12}:360:1) -- cycle;
        \draw ({basket*6}:.7) node{{ {_("Basket")} }};
        \draw ({basket*12+tennis*6}:.7) node{{ {_("Tennis")} }};
        \draw ({(basket+tennis)*12+football*6}:.7) node[white]{{ {_("Football")} }};
        \draw ({(basket+tennis+football)*12+judo*6}:.7) node{{ {_("Judo")} }};
    \end{{tikzpicture}}
    \end{{center}}
    \end{{minipage}}
    """)
